...
export default function CheckoutPage() {
    ...
    return (
      <div className="min-h-screen bg-[#151230] flex items-center justify-center relative overflow-hidden">
        ...
        <div className="text-center max-w-md mx-auto p-10 animate-fadeInUp relative z-10 glass rounded-3xl border border-white/10 shadow-2xl">
          <div className="w-24 h-24 rounded-full bg-[var(--color-accent)] flex items-center justify-center mx-auto mb-8 animate-pulse-soft shadow-lg shadow-[var(--color-accent)]/20">
            <FiCheck size={48} className="text-black" />
          </div>
          ...
          <div className="flex flex-col gap-4">
            <a href={shopInfo.socialMedia.whatsapp} target="_blank" rel="noreferrer"
              className="flex items-center justify-center gap-2 w-full py-4 rounded-xl bg-white text-black font-black text-sm hover:bg-gray-100 transition-all shadow-xl shadow-white/5">
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface)] text-gray-200">
      {/* Page Header */}
      <div className="bg-[var(--color-primary)] backdrop-blur-xl border-b border-white/10 py-6 relative overflow-hidden">
        <div className="absolute inset-0 bg-[var(--color-accent)]/5 pointer-events-none" />
        <div className="container-custom relative z-10">
          <Link to="/cart" className="inline-flex items-center gap-2 text-sm text-[var(--color-accent-light)] hover:text-white transition-all mb-3 font-semibold">
            <FiArrowLeft size={16} /> Back to Cart
          </Link>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Secure Checkout</h1>
        </div>
      </div>
      ...
      <div className="container-custom py-6">
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="p-8 rounded-3xl bg-[var(--color-surface-card)] border border-white/10 shadow-xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input name="fullName" value={formData.fullName} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all font-medium" required />
                </div>
                <div>
                  <label className="text-sm font-bold text-gray-400 mb-2 block">Phone Number *</label>
                  <input name="phone" value={formData.phone} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all font-medium" required />
                </div>
                <div className="sm:col-span-2">
                  <label className="text-sm font-bold text-gray-400 mb-2 block">Email Address</label>
                  <input name="email" type="email" value={formData.email} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all font-medium" />
                </div>
                <div className="sm:col-span-2">
                  <label className="text-sm font-bold text-gray-400 mb-2 block">Complete Delivery Address *</label>
                  <textarea name="address" value={formData.address} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all min-h-[100px] resize-none font-medium" required placeholder="House No, Street Name..." />
                </div>
                <div>
                  <label className="text-sm font-bold text-gray-400 mb-2 block">City / Town</label>
                  <input name="city" value={formData.city} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all font-medium" />
                </div>
                <div>
                  ...
                  <input name="postalCode" value={formData.postalCode} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all font-medium" />
                </div>
                <div className="sm:col-span-2">
                  ...
                  <textarea name="notes" value={formData.notes} onChange={handleChange} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[var(--color-accent)] focus:ring-1 focus:ring-[var(--color-accent)] transition-all min-h-[80px] resize-none font-medium text-sm" placeholder="Any special instructions for delivery..." />
                </div>
              </div>
            </div>

            {/* Payment Info */}
            <div className="p-8 rounded-3xl bg-[var(--color-surface-card)] border border-white/10 shadow-xl overflow-hidden relative">
              ...
              <div className="p-6 rounded-2xl bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/20 relative z-10 group">
                ...
                <div className="space-y-3 bg-black/20 p-5 rounded-xl border border-white/5 mb-4">
                  ...
                  <div className="flex justify-between items-center pb-2 border-b border-white/5">
                    ...
                    <span className="text-base font-mono font-black text-[var(--color-gold)]">{shopInfo.bankDetails.accountNumber}</span>
                  </div>
                ...
                <p className="text-xs text-[var(--color-accent-light)]/80 leading-relaxed font-medium">
          ...
          <div className="lg:sticky lg:top-28">
            <div className="bg-[var(--color-surface-card)] border border-white/10 rounded-3xl overflow-hidden shadow-2xl relative p-6">
              ...
              <button 
                ...
                className={`w-full py-4 rounded-xl bg-[var(--color-accent)] text-black font-bold text-base shadow-lg shadow-[var(--color-accent)]/20 transition-all flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[var(--color-accent-light)] hover:scale-[1.02] active:scale-95'}`}
              >
                {isSubmitting ? (
                  <>
