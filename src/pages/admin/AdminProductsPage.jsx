...
export default function AdminProductsPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    image_url: '', imageFile: null, imagePreview: '', sizes: '', colors: '', stock: '', featured: false
  })
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', price: '', discount_price: '', category: 'watches', image_url: '', imageFile: null, imagePreview: '', sizes: '', colors: '', stock: '', featured: false })
  }
  ...
  const handleSubmit = async (e) => {
    ...
    setUploading(true)
    let finalImageUrl = formData.image_url.trim() || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=600'
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-130px)]">
                ...
                <div className="space-y-3">
                  ...
                  {(formData.imagePreview || formData.image_url) && (
                    <div className="mt-4 w-32 h-32 rounded-xl overflow-hidden border border-[var(--color-border)] shadow-lg shadow-black/20 bg-black/20 flex items-center justify-center">
                      <img src={formData.imagePreview || formData.image_url} alt="Preview" className="max-w-full max-h-full object-contain" onError={e => e.target.style.display='none'} />
                    </div>
                  )}
                </div>
              ...
              <div className="flex gap-3 mt-6 pt-4 border-t border-[var(--color-border)]">
                ...
                <button type="submit" disabled={uploading} className="btn-primary flex-1 py-3 shadow-lg shadow-[var(--color-accent)]/20 disabled:opacity-50">
                  {uploading ? 'Processing...' : editProduct ? (
