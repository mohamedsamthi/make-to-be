...
import { FiPlus, FiEdit, FiTrash2, FiX, FiCheck, FiVideo } from 'react-icons/fi'
...
export default function AdminPromotionsPage() {
  ...
  const [formData, setFormData] = useState({
    title: '', descriptionText: '', mediaUrl: '', discount_percentage: '', active: true, banner_color: bannerColors[0]
  })

  const openAdd = () => {
    setEditPromo(null)
    setFormData({ title: '', descriptionText: '', mediaUrl: '', discount_percentage: '', active: true, banner_color: bannerColors[0] })
    setShowForm(true)
  }
  ...
  const openEdit = (p) => {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
         ...
         parsedMedia = parsed.mediaUrl || ''
      }
    } catch(e) {
      // Not JSON, assume plain text
    }
    setFormData({ title: p.title, descriptionText: parsedText, mediaUrl: parsedMedia, discount_percentage: String(p.discount_percentage), active: p.active, banner_color: p.banner_color })
    setShowForm(true)
  }
  ...
  const handleSubmit = (e) => {
    ...
    // Pack description text and media inside the description field as JSON
    const packedDescription = JSON.stringify({
      text: formData.descriptionText.trim(),
                      ...
                      {(() => {
                        try {
                          ...
                          return (
                            <>
                              {parsed.text}
                              {parsed.mediaUrl && <span className="block mt-1 text-xs text-blue-200"><FiVideo className="inline mr-1"/> Media attached</span>}
                            </>
                          )
                        } catch(e) {
          ...
          <div className="w-full max-w-md bg-[var(--color-surface-card)] border border-[var(--color-border)] rounded-2xl animate-fadeInUp" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input value={formData.descriptionText} onChange={e => setFormData(p => ({...p, descriptionText: e.target.value}))} className="input-field" placeholder="Sale details..." />
              </div>
              <div>
                <label className="text-sm font-medium text-[var(--color-text-secondary)] mb-1.5 block">Video / Media URL (Optional)</label>
                <input value={formData.mediaUrl} onChange={e => setFormData(p => ({...p, mediaUrl: e.target.value}))} className="input-field" placeholder="https://youtube.com/... or https://image.url" />
                <p className="text-xs text-[var(--color-text-muted)] mt-1">Paste a YouTube link or a direct image/video URL</p>
              </div>
              <div>
